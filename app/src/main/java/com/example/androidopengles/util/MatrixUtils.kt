package com.example.androidopengles.util
        ...
        if (imgHeight > 0 && imgWidth > 0 && viewWidth > 0 && viewHeight > 0) {
            ...
            if (sWhImg > sWhView) {
                when (type) {
                    ...
                    TYPE_FITEND -> Matrix.orthoM(projection, 0, -1f, 1f, -1f, 2 * sWhImg / sWhView - 1f, 1f, 3f)
                }
            } else {
                when (type) {
                    ...
                    TYPE_FITSTART -> Matrix.orthoM(projection, 0, -1f, 2 * sWhView / sWhImg - 1f, -1f, 1f, 1f, 3f)
